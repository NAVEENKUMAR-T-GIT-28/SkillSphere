...
describe('Students Routes', () => {
  ...
  describe('GET /api/students/:studentId/profile', () => {
    test('owner can view their own profile', async () => {
      const res = await request(app)
        .get(`/api/students/${student1._id}/profile`)
        .set('Authorization', `Bearer ${token1}`);
      expect(res.status).toBe(200);
      expect(res.body.data.full_name).toBe('Test Student');
    });

    test('faculty can view student profile', async () => {
      const res = await request(app)
        .get(`/api/students/${student1._id}/profile`)
        .set('Authorization', `Bearer ${facultyToken}`);
      expect(res.status).toBe(200);
    ...
    test('student cannot view another student profile', async () => {
      const res = await request(app)
        .get(`/api/students/${student1._id}/profile`)
    ...
    test('returns 404 for non-existent profile ID', async () => {
      const fakeId = new mongoose.Types.ObjectId();
      const res = await request(app)
        .get(`/api/students/${fakeId}/profile`)
        .set('Authorization', `Bearer ${facultyToken}`);
      expect(res.status).toBe(404);
    });
  });

  describe('PATCH /api/students/:studentId/profile', () => {
    test('owner can update their profile', async () => {
      const res = await request(app)
        .patch(`/api/students/${student1._id}/profile`)
        .set('Authorization', `Bearer ${token1}`)
        .send({ career_objective: 'Become a great dev' });
      expect(res.status).toBe(200);
      expect(res.body.data.career_objective).toBe('Become a great dev');
    });

    test('fails on invalid CGPA', async () => {
      const res = await request(app)
        .patch(`/api/students/${student1._id}/profile`)
        .set('Authorization', `Bearer ${token1}`)
        .send({ cgpa: 11 });
